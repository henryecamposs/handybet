...
export default function FriendsScreen() {
  ...
  return (
    <ScrollView className="flex-1 bg-background px-4 pt-12" showsVerticalScrollIndicator={false}>
      {/* Header */}
      <View className="mb-6">
        <Text className="text-foreground font-bold text-2xl">Amigos</Text>
        <Text className="text-foreground text-sm mt-1">Administra tus conexiones y encuentra nuevos amigos.</Text>
      </View>
      ...
      <View className="mb-8">
        <Text className="text-foreground font-bold text-lg mb-4">Solicitudes (2)</Text>
        {handyBetUsers.slice(0, 2).map((user) => (
          <View key={user.id} className="bg-background p-4 rounded-2xl border border-zinc-800 mb-3 flex-row items-center justify-between">
            <TouchableOpacity onPress={() => router.push(`/profile/${user.id}` as any)} className="flex-row items-center flex-1">
              ...
              <View>
                ...
                <Text className="text-foreground text-xs">Te ha enviado una solicitud</Text>
              </View>
            </TouchableOpacity>
            <View className="flex-row gap-2">
              ...
              <TouchableOpacity className="bg-background/80 px-4 py-2 rounded-full border border-zinc-700">
                <Text className="text-foreground font-bold text-xs">Ignorar</Text>
              </TouchableOpacity>
            </View>
          </View>
        ))}
      ...
      {/* Todos los amigos */}
      <View className="mb-8">
        <Text className="text-foreground font-bold text-lg mb-4">Todos tus amigos ({handyBetUsers.length})</Text>
          ...
          {handyBetUsers.map((user, index) => (
            <View key={user.id} className={`flex-row items-center justify-between p-4 ${index !== handyBetUsers.length - 1 ? 'border-b border-zinc-800/50' : ''}`}>
              ...
              <View className="flex-row gap-2">
                ...
                <TouchableOpacity className="w-9 h-9 rounded-full bg-background/80 items-center justify-center border border-zinc-700 hover:bg-background">
