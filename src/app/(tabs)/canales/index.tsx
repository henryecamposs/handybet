  ...
  return (
    <ScrollView className="flex-1 bg-zinc-950 px-4 pt-12" showsVerticalScrollIndicator={false}>
      ...
      <View className="flex-row items-center justify-between mb-6">
        <View>
          <Text className="text-zinc-100 font-bold text-2xl">Canales</Text>
          <Text className="text-zinc-400 text-sm mt-1">Directorio de empresas y consorcios.</Text>
        </View>
      </View>
      ...
      <View className="mb-8">
        <Text className="text-zinc-100 font-bold text-lg mb-4">Mis Canales</Text>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} className="flex-row">
          ...
          >
            ...
            <Text className="text-zinc-300 font-bold text-sm text-center">Crear Nuevo</Text>
          </TouchableOpacity>
          ...
          {misCanales.map((channel) => (
            ...
            >
              ...
              <Text className="text-zinc-100 font-bold text-center text-sm px-1" numberOfLines={2}>{channel.name}</Text>
      ...
      {isLoading ? (
        ...
        <View className="flex-1 items-center justify-center py-20 mt-4">
          ...
          <Text className="text-zinc-400 font-bold text-lg text-center">Aún no has creado canales</Text>
          <Text className="text-zinc-500 text-sm text-center mt-2 max-w-[250px]">Crea tu propio canal oficial para organizar a tus usuarios y ofrecer servicios de apuestas.</Text>
        </View>
      ) : (
        <View className="mb-8 mt-4">
          <Text className="text-zinc-100 font-bold text-lg mb-4">Directorio de Canales</Text>
          <View className="space-y-3">
            {channels.slice(1).map((channel) => (
              <TouchableOpacity
                key={channel.id}
                onPress={() => router.push(`/(tabs)/canales/${channel.id}` as any)}
                className="bg-zinc-900 border border-zinc-800 p-4 rounded-2xl flex-row justify-between items-center hover:bg-zinc-800/80 transition-colors"
              >
                <View className="flex-row items-center flex-1">
                  ...
                  <View className="flex-1">
                    <Text className="text-zinc-100 font-bold text-base" numberOfLines={1}>{channel.name}</Text>
                    <Text className="text-zinc-500 text-xs mt-1" numberOfLines={1}>Empresa de Apuestas Aliada</Text>
                  </View>
                </View>
                <View className="w-8 h-8 rounded-full bg-zinc-800 items-center justify-center border border-zinc-700 ml-2">
